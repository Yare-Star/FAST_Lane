...
def get_optimizer(net, cfg):
    ...
    if cfg.optimizer == 'Adam':
        ...
        # weight_decay 权重衰减（L2正则化，防止模型过拟合）
...
def get_scheduler(optimizer, cfg, iters_per_epoch):  # iters_per_epoch = len(train_loader)
...
class MultiStepLR:
    ...
    def step(self, external_iter = None):
        ...
        # 学习率预热
        if self.warmup == 'linear' and self.iters < self.warmup_iters:  # 还处在预热阶段
        ...
        if self.iters % self.iters_per_epoch == 0:
            ...
            for group, lr in zip(self.optimizer.param_groups, self.base_lr):  # 学习率两边小（0，24）epoch&&（39，-1）epoch，中间大(5,38) 由power控制
