...
# from model.model import parsingNet  # 原始model
from model.model_changed import parsingNet  # 修改后的model
...
def train(net, data_loader, loss_dict, optimizer, scheduler,logger, epoch, metric_dict, use_aux):
    ...
    for b_idx, data_label in enumerate(progress_bar):
        ...
        loss.backward()
        optimizer.step()
        scheduler.step(global_step)  # 修改学习率
        t_net_1 = time.time()
        ...
        if hasattr(progress_bar, 'set_postfix'):
            ...
            progress_bar.set_postfix(loss='%.3f' % float(loss), epoch=epoch,
...
if __name__ == "__main__":
    ...
    net = parsingNet(backbone=cfg.backbone, cls_dim=(cfg.griding_num+1, cls_num_per_lane, cfg.num_lanes), use_aux=cfg.use_aux).cuda()
    # 多卡加速
